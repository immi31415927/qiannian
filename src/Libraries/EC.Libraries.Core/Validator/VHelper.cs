...
namespace EC.Libraries.Core.Validator
{
    /// <summary>
    /// 验证类型
    /// </summary>
    ...
    public static class VHelper
    {
            ...
            if (!string.IsNullOrWhiteSpace(m))
            {
                switch (v)
                {
                    ...
                    case VType.Bank:
                        check = IsBank(m);
                        break;
                    case VType.IsLetterAndNumber:
                        check = IsLetterAndNumber(m);
                        break;
                }
            }
            return check;
        ...
        {
            return Regex.IsMatch(m_account, "^[a-zA-Z][a-zA-Z0-9_]{1,15}$");
        }

        /// <summary>
        /// 验证输入的银行卡号
        /// </summary>
        /// <param name="m_account">银行卡号</param>
        /// <returns>有效：true 无效：false</returns>
        /// <remarks>2013-12-30 苟治国 注释</remarks>
        private static bool IsBank(string m_account)
        {
            //return Regex.IsMatch(m_account, "^\d{16}|\d{19}$");
            return true;
        }
        ...
        /// 字母数字验证
        /// </summary>
        /// <param name="m_letterAndNumber">银行卡号</param>
        /// <returns>有效：true 无效：false</returns>
        /// <remarks>2013-12-30 苟治国 注释</remarks>
        private static bool IsLetterAndNumber(string m_letterAndNumber)
        {
            return Regex.IsMatch(m_letterAndNumber, "^[a-zA-Z0-9]+$");
        }
